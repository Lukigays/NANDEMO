...
const prosesrespone = (target, ctx) => {
    const photoUrl = 'https://files.catbox.moe/6mbpx2.jpg'; // Ganti dengan URL gambar atau gunakan buffer gambar
...
const donerespone = (target, ctx) => {
    const photoUrl = 'https://files.catbox.moe/zdi4u1.jpg'; // Ganti dengan URL gambar atau gunakan buffer gambar
...
bot.command("play", async (ctx) => {
    ...
    try {
        ...
        const apiResponse = await axios.get(`https://api.siputzx.my.id/api/d/youtube?q=${encodeURIComponent(res.url)}`);
...
bot.start(ctx => {
  const menuMessage = `
👋hello, welcome , press the button below to bring up the menu       

Selamat ${greeting} !
╭━━━「 🌸WELCOME🌸 」━━━⬣
│ 
│ Yo 😉! 
│ Selamat datang di bugbotlukyy
│
├━━━「 📑 FITUR 」━━━⬣
│ CLICK TOMBOLNYA
╰━━━━━━━━━━━━━━━━⬣
...
`;
  ...
  const photoUrl = "https://files.catbox.moe/d4gcxh.jpg"; 


const keyboard = [
    [
        { text: "BUGMENU", callback_data: "bugmenu" },
        { text: "☁️ Support Owner", url: "https://t.me/lukyycihuyy" }
    ],
    [
...
bot.action("ownermenu", (ctx) => {
  ...
  const menu = `
   👋 Hello Unknown, I am the 𝙏𝙚𝙡𝙚𝙜𝙧𝙖𝙢 𝘽𝙤𝙩. I'm here to assist you with anything you might need, making your interaction smoother and more efficient.       
Selamat ${greeting} !
ᝄ ⌜ 𝙊 𝙬 𝙣 𝙚 𝙧 𝙈 𝙚 𝙣 𝙪 ⌟
䒘 > /delprem
䒘 > /addprem
...
> © 1.1
  ...
  ctx.replyWithPhoto("https://files.catbox.moe/d4gcxh.jpg", {
    caption: menu,
    parse_mode: "Markdown",
    reply_markup: {
      inline_keyboard: keyboard
    ...
    ctx.replyWithAudio({
      url: "https://files.catbox.moe/hqtdrj.mp3" 
    });
  });
});
bot.action("thaksto", (ctx) => {
  ...
  const greeting = new Date().getHours() < 12 ? "Pagi" : new Date().getHours() < 18 ? "Sore" : "Malam";
  const menu = `
   👋 Hello Unknown, this is a creadit from our sc ver 1.1 thanks to @caywzz because I used the base and sorry if this script is bad because it's only in the development stage.       
